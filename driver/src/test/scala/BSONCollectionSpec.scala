...
class BSONCollectionSpec(implicit ee: ExecutionEnv)
  ...
  "BSON collection" should {
    ...
    "read documents skipping errors using collect" >> {
      implicit val reader = new SometimesBuggyPersonReader
      def resultSpec(c: BSONCollection, timeout: FiniteDuration) = findAll(c).cursor[Person]().collect[Vector](stopOnError = false).
    ...
    "write a document with error" >> {
      ...
      def writeSpec(c: BSONCollection, timeout: FiniteDuration) = c.insert(person).map { lastError =>
...

    {
      ...
      "with default connection" >> {
        "find and remove 'Joline' using findAndModify" in {
        ...
        "find and remove 'Foo' using findAndRemove" in {
