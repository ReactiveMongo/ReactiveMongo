    ...
    "be written" >> {
      ...
      "with correct `deletes` elements" in {
        ...
        withDelete(cmd)(element :: Nil) { delete =>
          cmd.pack.serialize(delete, cmd.deleteWriter) must_=== (
            base ++ writeConcern ++ deletes(bson :: Nil))
        }
      }

      lazy val session = new NodeSetSession(java.util.UUID.randomUUID())

      lazy val lsid = BSONDocument(
        "lsid" -> BSONDocument(
            ...
            session.startTransaction(WriteConcern.Default, None).
              aka("transaction") must beSuccessfulTry[(SessionTransaction, Boolean)].which { _ =>
                ...
                write(delete) must_=== (base ++ lsid ++ BSONDocument(
                  ...
                  "autocommit" -> false) ++ dels)
              }
          } and {
            // w/o 'startTransaction' flag after first command in tx

            write(delete) must_=== (base ++ lsid ++ BSONDocument(
              "txnNumber" -> 1L, "autocommit" -> false) ++ dels)
          }
        }
      }
    }
