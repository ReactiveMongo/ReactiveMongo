...
trait UpdateSpec extends UpdateFixtures { collectionSpec: CollectionSpec =>
  ...
  def updateSpecs = {
    ...
    "update many documents" in {
      ...
      def resultSpec(res: Future[MultiBulkWriteResult]) =
        res must beLike[MultiBulkWriteResult] {
          case result => result.n must_=== 3 and {
            result.nModified must_=== 0 // upserted
          } and {
            coll.find(BSONDocument.empty).cursor[BSONDocument]().
              collect[Set](5, Cursor.FailOnError[Set[BSONDocument]]()).
              aka("collected") must beTypedEqualTo(
      ...
      update.maxBulkSize must_=== 2 and {
        Future.sequence(Seq(
          ...
          aka("elements") must beLike[Seq[coll.UpdateCommand.UpdateElement]] {
            case elements @ (up1 +: _) =>
              elements.size must_=== 3 and {
                ...
                coll.count() must beTypedEqualTo(0).await(1, timeout)
              } and {
