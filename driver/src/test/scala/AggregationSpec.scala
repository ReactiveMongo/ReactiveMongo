...
final class AggregationSpec(implicit ee: ExecutionEnv)
  ...
  lazy val coll: BSONCollection = {
    ...
    scala.concurrent.Await.result(
      c.create()
        .flatMap(_ =>
          c.indexesManager
            .ensure(index(List("city" -> Text, "state" -> Text)))
            .map(_ => c)
        ),
      timeout * 2
    )
  ...
  private val jpCodes = List(
    ZipCode("JP 13", "TOKYO", "JP", 13185502L, Location(35.683333, 139.683333)),
  ...
  "Zip codes collection" should {
    "expose the index stats" in {
      ...
      val idxAgg = coll
        .aggregateWith[IndexStatsResult]() { framework =>
          import framework._
        ...
        .collect[List](
          Int.MaxValue,
          Cursor.FailOnError[List[IndexStatsResult]]()
        )
    ...
    "return states with populations above 10000000" >> {
      ...
      def withRes[T](
          c: BSONCollection
        )(f: Future[List[BSONDocument]] => T
        ): T = {
        f(
          c.aggregateWith[BSONDocument]() { framework =>
      ...
      "with expected count" in {
        ...
        val result = coll
          .aggregatorContext[BSONDocument](
    ...
    "explain simple result" in {
      val result = coll
        .aggregateWith[BSONDocument](explain = true) { framework =>
          import framework.{ Group, Match, SumField }

          List(
            Group(BSONString(f"$$state"))("totalPop" -> SumField("population")),
            Match(document("totalPop" -> document(f"$$gte" -> 10000000L)))
          )
        }
        .headOption
    ...
    "return average city population by state" >> {
      ...
      def withCtx[T](
          c: BSONCollection
        )(f: (c.AggregationFramework.Group, List[c.PipelineOperator]) => T
        ): T = {
        import c.AggregationFramework
        import AggregationFramework.{ Ascending, Group, Sort, SumField }

        val firstOp = Group(
          document("state" -> f"$$state", "city" -> f"$$city")
      ...
      "successfully as a single batch" in {
        withCtx(coll) { (firstOp, pipeline) =>
          coll
            .aggregateWith[BSONDocument]() { _ => firstOp +: pipeline }
      ...
      "with cursor" >> {
        ...
        "with metadata sort" in {
          coll
            .aggregateWith[ZipCode]() { framework =>
              import framework.{
                Descending,
                Match,
                MetadataSort,
              ...
              val firstOp = Match(
                BSONDocument(f"$$text" -> BSONDocument(f"$$search" -> "JP"))
              )
              val pipeline =
                List(Sort(MetadataSort("score", TextScore), Descending("city")))
      ...
      "with produced cursor" >> {
        "without limit (maxDocs)" in withCtx(coll) { (firstOp, pipeline) =>
          ...
          cursor.collect[List](
            Int.MaxValue,
            Cursor.FailOnError[List[BSONDocument]]()
          ) must beTypedEqualTo(expected).await(1, timeout)
        }
    ...
    "return largest and smallest cities by state" in {
      ...
      val groupPipeline = List(
        ...
        Project(
          document(
            ...
            "biggestCity" -> document(
              "name" -> f"$$biggestCity",
              "population" -> f"$$biggestPop"
            ),
            "smallestCity" -> document(
              "name" -> f"$$smallestCity",
              "population" -> f"$$smallestPop"
            )
          )
        ),
      ...
      coll
        .aggregateWith[BSONDocument]() { _ => primGroup +: groupPipeline }
        .collect[List](
          Int.MaxValue,
          Cursor.FailOnError[List[BSONDocument]]()
        ) must beTypedEqualTo(expected).await(1, timeout) and {
        coll
          .aggregateWith[BSONDocument]() { _ =>
            primGroup +: (groupPipeline :+ Limit(2))
          }
          .collect[List](
            Int.MaxValue,
            Cursor.FailOnError[List[BSONDocument]]()
          ) must beTypedEqualTo(expected take 2).await(1, timeout)
      } and {
        coll
          .aggregateWith[BSONDocument]() { _ =>
          ...
          .collect[List](
            Int.MaxValue,
            Cursor.FailOnError[List[BSONDocument]]()
          ) must beTypedEqualTo(expected drop 2).await(1, timeout)
      }
    }
    ...
    "return distinct states" >> {
      def distinctSpec(c: BSONCollection, timeout: FiniteDuration) =
        c.distinct[String, ListSet](
          ...
          collation = None
        ) must beTypedEqualTo(ListSet("NY", "FR", "JP")).await(1, timeout)
  ...
  "Inventory #1" should {
    ...
    "perform a simple lookup so the joined documents are returned" in {
      ...
      def expected = List(
        ...
        InventoryReport(
          3,
          docs = List(Product(5, None, Some("Incomplete")), Product(6))
        )
      )
      ...
      orders
        .aggregatorContext[InventoryReport](
  ...
  f"Aggregation result for '$$out'" should {
    ...
    "be outputed to collection" in {
      ...
      books
        .aggregateWith[Author]() { framework =>
          import framework._
        ...
        db.collection[BSONCollection](outColl)
          ...
          .collect[List](
            3,
            Cursor.FailOnError[List[Author]]()
          ) must beTypedEqualTo(
          List(
  ...
  "Aggregation result for '$stdDevPop'" should {
    ...
    "with valid fixtures" in {
      ...
      val fixtures = Seq(
        ...
        BSONDocument("_id" -> 2, "name" -> "dave2", "quiz" -> 1, "score" -> 90),
        BSONDocument("_id" -> 3, "name" -> "ahn", "quiz" -> 1, "score" -> 71),
        BSONDocument("_id" -> 4, "name" -> "li", "quiz" -> 2, "score" -> 96),
        BSONDocument("_id" -> 5, "name" -> "annT", "quiz" -> 2, "score" -> 77),
        BSONDocument("_id" -> 6, "name" -> "ty", "quiz" -> 2, "score" -> 82)
      )
    ...
    "return the standard deviation of each quiz" in {
       ...
       */
      contest
        .aggregateWith[QuizStdDev]() { framework =>
          import framework._

          List(
            Group(BSONString(f"$$quiz"))("stdDev" -> StdDevPopField("score")),
            Sort(Ascending("_id"))
          )
        }
    ...
    "return a sum as hash per quiz" in {
      ...
      contest
        .aggregatorContext[BSONDocument](
          List(
            Group(BSONString(f"$$quiz"))(
              "hash" -> Sum(
                document(f"$$multiply" -> array(f"$$_id", f"$$score"))
              )
            )
          )
        )
        ...
        .collect[Set](
          Int.MaxValue,
          Cursor.FailOnError[Set[BSONDocument]]()
        ) must beTypedEqualTo(
        Set(
    ...
    "return the maximum score per quiz" in {
      ...
      contest
        .aggregatorContext[BSONDocument](
          List(Group(BSONString(f"$$quiz"))("maxScore" -> MaxField("score")))
        )
        .prepared
        .cursor
        .collect[Set](
    ...
    "return the minimum score per quiz" in {
      ...
      contest
        .aggregatorContext[BSONDocument](
          List(Group(BSONString(f"$$quiz"))("minScore" -> MinField("score")))
        )
        .prepared
        .cursor
        .collect[Set](
    ...
    "push name and score per quiz group" in {
      val expected = Set(
        QuizScores(2, Set(Score("ty", 82), Score("annT", 77), Score("li", 96))),
        QuizScores(
          1,
          Set(Score("dave123", 85), Score("dave2", 90), Score("ahn", 71))
        )
      )
      ...
      contest
        ...
        .collect[Set](
          Int.MaxValue,
          Cursor.FailOnError[Set[QuizScores]]()
        ) must beTypedEqualTo(expected).await(1, timeout)
    }
  ...
  "Geo-indexed documents" should {
    ...
    "and aggregated using $geoNear" in {
      ...
      implicit val pointReader: BSONDocumentReader[GeoPoint] =
        Macros.reader[GeoPoint]
      implicit val distanceReader: BSONDocumentReader[GeoDistance] =
        BSONDocumentReader[GeoDistance] { doc =>
          (for {
            calc <- doc.double("calculated").map(_.toInt)
            loc <- doc.getAsOpt[GeoPoint]("loc")
          } yield GeoDistance(calc, loc)).get
        }
      implicit val placeReader: BSONDocumentReader[GeoPlace] =
        Macros.reader[GeoPlace]

      import places.AggregationFramework.{ GeoNear, Limit }

      places
        .aggregatorContext[GeoPlace](
          List(
            GeoNear(
              document(
                "type" -> "Point",
        ...
        .collect[List](
          Int.MaxValue,
          Cursor.FailOnError[List[GeoPlace]]()
        ) aka "places" must beTypedEqualTo(
        List(
          GeoPlace(
...
db.forecasts.aggregate(
   [
     ...
     { $redact: {
       ...
       */
      val result = forecasts
        .aggregateWith[Redaction]() { framework =>
...
db.accounts.aggregate([
      ...
      fruits.insert
        .one(
          document(
            ...
            "in_stock" -> document("oranges" -> 20, "apples" -> 60),
  ...
  "Users" should {
        ...
        .many(
          Seq(
            ...
            User(
              _id = 2,
              name = "li",
              favorites = Seq("apples", "pudding", "pie")
            ),
            User(
              _id = 3,
              name = "ahn",
    ...
    f"be aggregated using $$slice and $$count" in {
      users
        ...
        .aka("top favorites") must beTypedEqualTo(
        Seq(
          User(
            _id = 1,
            name = "dave123",
  ...
  "Stage" should {
    ...
    f"be $$facet" in {
      ...
      makePipe(
        ...
        f"$$facet" -> BSONDocument(
          ...
          "lorem" -> BSONArray(BSONDocument(f"$$out" -> "ipsum"))
        )
      )
    }
    ...
    f"be $$changeStream" >> {
      ...
      "with fullDocument strategy" >> {
        Fragments.foreach[St](Seq(St.Default, St.UpdateLookup)) { st =>
          st.name in {
            makePipe(
              ChangeStream(fullDocumentStrategy = Some(st))
            ) must_=== BSONDocument(
      ...
      "with offset" >> {
        "that startAfter" in {
          makePipe(
            ChangeStream(offset =
              Some(ChangeStream.StartAfter(BSONString("offset")))
            )
          ) must_=== BSONDocument(
        ...
        "that startAtOperationTime" in {
          makePipe(
            ChangeStream(offset = Some(ChangeStream.StartAt(1234L)))
          ) must_=== BSONDocument(
        ...
        "that resumeAfter" in {
          makePipe(
            ChangeStream(offset =
              Some(ChangeStream.ResumeAfter(BSONString("offset")))
            )
          ) must_=== BSONDocument(
      ...
      "with both fullDocument strategy and offset" in {
        makePipe(
          ChangeStream(
            ...
            fullDocumentStrategy = Some(St.Default)
          )
        ) must_=== BSONDocument(
          f"$$changeStream" -> BSONDocument(
            "startAtOperationTime" -> BSONTimestamp(1234L),
    ...
    f"be $$graphLookup" in {
      ...
      makePipe(
        GraphLookup(
          ...
          restrictSearchWithMatch = Some(BSONDocument.empty)
        )
      ) must_=== BSONDocument(
        f"$$graphLookup" -> BSONDocument(
          "from" -> "foo",
          "startWith" -> 1,
          "connectFromField" -> "bar",
          "connectToField" -> "lorem",
    ...
    f"be $$lookup with pipeline" in {
      ...
      val let =
        BSONDocument("order_item" -> f"$$item", "order_qty" -> f"$$ordered")
      ...
      "with explicit target DB" in {
        makePipe(
          ...
          f"$$merge" -> BSONDocument(
            "into" -> BSONDocument("db" -> "foo", "coll" -> "bar"),
            "on" -> Seq("lorem", "ipsum"),
            "whenMatched" -> "replace",
  ...
  "Group accumulator" >> {
    ...
    Fragments.foreach(
      Seq[(GroupFunction, BSONDocument)](
        ...
        MergeObjects(BSONDocument("lorem" -> 1)) -> BSONDocument(
        ...
        AddToSet(BSONString(f"$$bar")) -> BSONDocument(
          f"$$addToSet" -> f"$$bar"
        ),
      ...
      case (gfun, expected) =>
        gfun.getClass.getSimpleName in {
          reactivemongo.api.tests
            .makeFunction(AggFramework)(gfun) must_=== expected
        }
    }
  }
  ...
  "Atlas Search" >> {
    ...
    "serialize score for term search" >> {
      ...
      Fragments.foreach[(AtlasScore, BSONDocument)](
        Seq(
          AtlasScore.constant(1.23D) -> BSONDocument(
            "constant" -> BSONDocument("value" -> 1.23D)
          ),
          AtlasScore.boost(45.6D) -> BSONDocument(
            "boost" -> BSONDocument("value" -> 45.6D)
          )
        )
      ) {
        case (score, expected) =>
    ...
    val term1 =
      Term(query = "foo", path = "title" -> Seq("description", "tags"))

    val text1 =
      Text(query = "foo", path = "title" -> Seq("description", "tags"))

    val phrase1 =
      Phrase(query = "foo", path = "title" -> Seq("description", "tags"))
      ...
      "term" >> {
        ...
        "with multi analyzer in path" in {
          doc(
            ...
            "query" -> Seq("foo", "bar"),
            "path" -> Seq(
              BSONDocument("value" -> "title", "multi" -> "alternateAnalyzer"),
              string("anotherField")
            )
          )
        }
        ...
        "with score" in {
          doc(
            Term(
              query = "foo" -> Seq("bar"),
              path = "title" -> Nil,
              score = Some(AtlasScore.boost(2.1D))
            )
          ) must_=== BSONDocument(
            "query" -> Seq("foo", "bar"),
            "path" -> "title",
        ...
        Fragments.foreach[(Term.Modifier, String)](
          Seq(
          ...
          case (mod, flag) =>
            s"with simple modifier ${flag}" in {
              doc(
                Term(
                  query = "foo",
                  path = "title",
                  modifier = Some(mod),
                  score = Some(AtlasScore.constant(3.45D))
                )
              ) must_=== BSONDocument(
                "query" -> "foo",
                "path" -> "title",
        ...
        "with fuzzy modifier" in {
          doc(
            Term(
              query = "foo",
              path = "title",
              modifier = Some(Term.Fuzzy(maxEdits = 3, prefixLength = 2))
            )
          ) must_=== BSONDocument(
            "query" -> "foo",
            "path" -> "title",
            "fuzzy" -> BSONDocument("maxEdits" -> 3, "prefixLength" -> 2)
          )
      ...
      "compound" >> {
        ...
        "with minimal options" in {
          ...
          op.name must_=== "compound" and {
            ...
            doc(builder1.minimumShouldMatch(2).result()) must_=== expected and {
              doc(
                newBuilder(Compound.must, term1, 2).result()
              ) must_=== expected
            }
          }
        }
        ...
        "with multiple 'must' operators" in {
          ...
          doc(builder.result()) must_=== expected1 and {
            ...
            doc(builder.minimumShouldMatch(3).result()) must_=== expected2 and {
              doc(
                newBuilder(Compound.must, term1, Seq(text1), 3).result()
              ) must_=== expected2
            } and {
              doc(
                newBuilder(Compound.must, term1 -> Seq(text1), 3).result()
        ...
        "with multiple clauses" in {
          doc(
            builder1.append(Compound.mustNot, phrase1).result()
          ) must_=== BSONDocument(
            "must" -> Seq(BSONDocument("term" -> doc(term1))),
            "mustNot" -> Seq(BSONDocument("phrase" -> doc(phrase1)))
          ) and {
            val expected = BSONDocument(
              "must" -> Seq(BSONDocument("term" -> doc(term1))),
              "mustNot" -> Seq(
            ...
            doc(
              builder1.append(Compound.mustNot, text1, Seq(phrase1)).result()
            ) must_=== expected and {
              doc(
                builder1
        ...
        "with overriden clause" in {
          doc(
            builder1.append(Compound.must, phrase1).result()
          ) must_=== BSONDocument(
    ...
    f"be serialized as $$search" >> {
      ...
      "with compound operator" in {
        val builder = Compound
          .newBuilder(Compound.must, term1)
          .append(Compound.mustNot, text1 -> Seq(phrase1))

        makePipe(AtlasSearch(builder.result())) must_=== BSONDocument(
          f"$$search" -> BSONDocument(
            "compound" -> BSONDocument(
              "must" -> Seq(BSONDocument("term" -> doc(term1))),
              "mustNot" -> Seq(
                BSONDocument("text" -> doc(text1)),
                BSONDocument("phrase" -> doc(phrase1))
  ...
  "Update" should {
    "support pipeline as operator" in {
      ...
      BSON
        .writeDocument(ReplaceRootField("foo"))
        .aka("pipeline as update") must beSuccessfulTry(
        BSONDocument(f"$$replaceRoot" -> BSONDocument("newRoot" -> f"$$foo"))
      )
    }
  }
  ...
  private def index(
      ...
      options: pack.Document = builder.document(Seq.empty)
    ) = Index(pack)(
    key,
    name,
    unique,
