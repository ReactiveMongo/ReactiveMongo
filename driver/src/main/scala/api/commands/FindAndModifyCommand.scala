...
trait FindAndModifyCommand[P <: SerializationPack] {
  ...
  protected[reactivemongo] final type FindAndModifyCmd =
    ResolvedCollectionCommand[FindAndModify]
  ...
  implicit private[reactivemongo] lazy val findAndModifyWriter: pack.Writer[FindAndModifyCmd] = {
    ...
    pack.writer[FindAndModifyCmd] { cmd =>
      import builder.{ array, boolean, elementProducer => element, int, string }
      import cmd.command
      ...
      elements ++= Seq(
        ...
        element(
          ...
          boolean(command.bypassDocumentValidation)
        )
      )

      if (
        maxWireVersion.compareTo(MongoWireVersion.V40) >= 0 &&
      ...
      command.fields.foreach { f => elements += element("fields", f) }
      ...
      command.sort.foreach { s => elements += element("sort", s) }
      ...
      new FindAndModifyResult(
        decoder.child(result, "lastErrorObject").map { doc =>
          new FindAndUpdateLastError(
            decoder.booleanLike(doc, "updatedExisting").getOrElse(false),
