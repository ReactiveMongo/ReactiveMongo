...
trait MultiBulkWriteResultFactory[P <: SerializationPack] {
  self: PackSupport[P]
  ...
  final class MultiBulkWriteResult private (
    ...
    private[api] def merge(wr: WriteResult): MultiBulkWriteResult = wr match {
      case wr: UpdateWriteResult @unchecked =>
        new MultiBulkWriteResult(
          _ok = ok && wr.ok,
          _n = n + wr.n,
          _writeErrors = writeErrors ++ wr.writeErrors
            .map(e => e.copy(index = e.index + totalN)),
