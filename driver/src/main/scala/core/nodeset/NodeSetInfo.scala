...
final class NodeSetInfo private[reactivemongo] (
  ...
  private[reactivemongo] def withAwaitingRequests(count: Int, maxPerChannel: Int): NodeSetInfo = new NodeSetInfo(name, version, nodes, primary, mongos, secondaries, nearest, Some(count), Some(maxPerChannel), compression)
  ...
  private lazy val tupled = Tuple10(name, version, nodes, primary, mongos, secondaries, nearest, awaitingRequests, maxAwaitingRequestsPerChannel, compression)
