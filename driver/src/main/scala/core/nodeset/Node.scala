...
private[reactivemongo] case class Node(
  ...
  def toShortString = s"""Node[$name: $status (${authenticatedConnections.size}/${connected.size}/${connections.size} available connections), latency=${pingInfo.ping}ms, authenticated={${authenticated.map(_.toShortString) mkString ", "}}]"""
