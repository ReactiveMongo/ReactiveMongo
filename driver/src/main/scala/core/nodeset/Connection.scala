...
private[reactivemongo] class Connection(
  ...
  def send(
    ...
    compression: ListSet[Compressor]): ChannelFuture =
  ...
  def send(
    message: Request,
    compression: ListSet[Compressor]): ChannelFuture = withPrepared(
    compression, message) { (msg, onComplete) =>
  ...
  private def withPrepared(
    ...
    request: Request): Function2[Request, () => Any, ChannelFuture] => ChannelFuture =
  ...
  private[core] def copy(
    ...
    new Connection(
      channel, status, authenticated, authenticating, signaling)
