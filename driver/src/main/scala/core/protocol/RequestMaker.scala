...
private[reactivemongo] final class RequestMaker(
  ...
  private val callerSTE: Seq[StackTraceElement] = {
    ...
    current ++= reactivemongo.util.Trace.currentTraceElements
      .drop(3)
      .dropRight(6)
...
private[reactivemongo] object RequestMaker {
  ...
  def apply(
      ...
      callerSTE: Seq[StackTraceElement]
    ): RequestMaker =
    new RequestMaker(
      kind,
      op,
  ...
  def unapply(
    ...
    Some((maker.op, maker.payload, maker.readPreference, maker.channelIdHint))
